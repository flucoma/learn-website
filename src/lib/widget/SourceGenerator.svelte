<script>
	...
	export function init() {
		...
		loCluster = new Tone.Oscillator(loClusterFreq, 'sine').connect(cluster).start();
		hiCluster = new Tone.Oscillator(hiClusterFreq, 'sine').connect(cluster).start();

		filteredNoise = new Tone.Gain(1.0);
		filter = new Tone.Filter(filterFreq, 'bandpass').connect(filteredNoise);
		filter.set({ Q: 30 });
		filteredNoiseSource = new Tone.Noise('white').connect(filter).start();
	...
	<div id="source-parameters">
		{#if activeNode === sampler}
			<div class="samples">
				<button
					on:click={async () => {
						sampler.load('/audio/oboe-m.mp3').then(() => sampler.start());
					}}
				>
					oboe
				</button>
				<button
					on:click={async () => {
						sampler.load('/audio/bass-m.mp3').then(() => sampler.start());
					}}
				>
					bass
				</button>
				<button
					on:click={async () => {
						sampler.load('/audio/piano-m.mp3').then(() => sampler.start());
					}}
				>
					piano
				</button>
				<button
					on:click={async () => {
						sampler.load('/audio/Tremblay-AaS-VoiceQC-B2K.mp3').then(() => sampler.start());
					}}
				>
					voice
				</button>
		...
		{#if activeNode === osc}
			<label for="oscpos">
				...
				<input id="oscpos" type="range" bind:value={oscPos} min={0} max={1000} />
				{oscFreq.toFixed(0)}
			</label>
		{/if}
		...
		{#if activeNode === cluster}
			<label for="lowclusterpos">
				...
				<input id="lowclusterpos" type="range" bind:value={loClusterPos} min={0} max={1000} />
		...
		{#if activeNode === cluster}
			<label for="highclusterpos">
				...
				<input id="highclusterpos" type="range" bind:value={hiClusterPos} min={0} max={1000} />
		...
		{#if activeNode === filteredNoise}
			<label for="filterfreq">
				filter frequency
				<input id="filterfreq" type="range" bind:value={filterPos} min={0} max={1000} />

<script>
	...
	const randomcx = xpos.map(x => x * width).join(';');
	const randomcy = ypos.map(x => x * height).join(';');
...
<div class="container">
	...
	<svg class='multislider' width=400 height=400>
		{#each heights as h, i}
		<rect class={'slider-'+i} width={width/4} height={height} x={width/4 * i}>
			<animate 
